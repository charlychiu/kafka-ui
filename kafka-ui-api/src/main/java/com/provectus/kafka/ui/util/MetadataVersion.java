...
/**
 * Maps the KRaft {@code metadata.version} feature level (returned by
 * {@code AdminClient.describeFeatures()}) to a human-readable Kafka release.
 *
 * <p>In KRaft mode (Kafka 3.3+, mandatory since 4.0) the broker no longer exposes the
 * {@code inter.broker.protocol.version} config, so this feature level is the only reliable
 * signal of the cluster version available through the AdminClient.
 */
